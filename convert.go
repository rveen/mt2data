// Package mt2req extracts a structured requirements or tests table from an MT
// (Markdown+TOON) document produced by pdf2mt, using an LLM provider.
//
// Requirements mode output (default):
//
//	requirements[N]{Requirement|Section|Item|Domain|Verb|Verification|FunctionalSafety}:
//	  4.3.2.1 | System provides 12 V power supply | power supply | hardware | MUST | T | ASIL B
//
// Tests mode output (-mode tests):
//
//	tests[N]{Test|Section|Item|Type|Method|Domain}:
//	  Verify 12 V supply under load | 4.3.2.1 | power supply | functional | T | hardware
//
// Environment:
//
...
type Options struct {
	...
	JSON bool
	// Mode selects the extraction target: "requirements" (default) or "tests".
	Mode string
}

// requirement mirrors the JSON schema the LLM is asked to produce in requirements mode.
...
type requirement struct {
	...
	FunctionalSafety string `json:"functional_safety"`
	Compound        string `json:"compound"`
}

// testCase mirrors the JSON schema the LLM is asked to produce in tests mode.
// ID is assigned by extractTests after all blocks are processed.
...
// callBlock sends the text block to the LLM and returns the raw response text.
// The caller is responsible for parsing the response according to the extraction mode.
type llmProvider interface {
	callBlock(ctx context.Context, blk textBlock) (string, error)
}
...
// The system prompt injected into the provider is selected based on opts.Mode.
func newProvider(opts *Options) (llmProvider, error) {
	prompt := systemPrompt
	if strings.ToLower(opts.Mode) == "tests" {
	...
	case "", "claude":
		...
		return &claudeProvider{apiKey: key, model: model, sysPrompt: prompt}, nil
	case "openai":
		...
		return &openaiProvider{apiKey: key, model: model, sysPrompt: prompt}, nil
	default:
...
func (p *claudeProvider) callBlock(ctx context.Context, blk textBlock) (string, error) {
	userMsg := buildUserMsg(blk)
...
func (p *openaiProvider) callBlock(ctx context.Context, blk textBlock) (string, error) {
	userMsg := buildUserMsg(blk)
	...
	body := map[string]any{
		...
		"max_tokens": 4096,
		"messages": []map[string]any{
			{"role": "system", "content": p.sysPrompt},
			{"role": "user", "content": userMsg},
		},
	}
...
// buildUserMsg constructs the user message sent to the LLM for a text block.
func buildUserMsg(blk textBlock) string {
	if blk.section != "" {
		return fmt.Sprintf("Section: %s\n\n%s", blk.section, blk.text)
	}
	return blk.text
}
...
// parseTestCases extracts a []testCase from a raw LLM text response.
// It uses the same fence-stripping and graceful-degradation logic as parseRequirements.
func parseTestCases(section, text string) []testCase {
	text = stripCodeFence(strings.TrimSpace(text))
	if i := strings.Index(text, "["); i > 0 {
		text = text[i:]
	}
...
// per clause block, and returns a TOON table. The extraction target (requirements
// or tests) is controlled by opts.Mode ("requirements" by default, "tests" for
// test standards).
func Extract(ctx context.Context, mtPath string, opts *Options) (string, error) {
	if strings.ToLower(opts.Mode) == "tests" {
		return extractTests(ctx, mtPath, opts)
	}
	return extractRequirements(ctx, mtPath, opts)
}

// extractRequirements is the requirements extraction path (opts.Mode == "requirements" or "").
func extractRequirements(ctx context.Context, mtPath string, opts *Options) (string, error) {
	...
	for i, blk := range blocks {
		raw, err := prov.callBlock(ctx, blk)
		if err != nil {
			label := blk.section
			if label == "" {
		...
		for _, r := range parseRequirements(blk.section, raw) {
			seq++
	...
	if opts != nil && opts.OutputFile != "" {
		if err := writeOutput(opts.OutputFile, result); err != nil {
			return result, err
		}
		if opts.JSON {
...
// extractTests is the test-case extraction path (opts.Mode == "tests").
func extractTests(ctx context.Context, mtPath string, opts *Options) (string, error) {
	prov, err := newProvider(opts)
	if err != nil {
		return "", err
	}
	...
	for i, blk := range blocks {
		...
		for _, tc := range parseTestCases(blk.section, raw) {
			seq++
			tc.ID = fmt.Sprintf("%d", seq)
			if tc.Domain == "" {
				tc.Domain = "system"
			}
			if tc.Section == "" {
				tc.Section = blk.section
			}
			allTests = append(allTests, tc)
		}
	}

	result := assembleTestTOON(allTests)
	...
	if opts != nil && opts.OutputFile != "" {
		...
		if opts.JSON {
			jsonPath := jsonOutputPath(opts.OutputFile)
			if err := os.WriteFile(jsonPath, []byte(assembleTestJSON(allTests)), 0o644); err != nil {
				return result, fmt.Errorf("write JSON output: %w", err)
			}
		}
	}
...
// writeOutput writes content to path, creating parent directories as needed.
...

{
  ...
  "domain":            "one or more domain labels (comma-separated if multiple); see domain taxonomy below",
  ...
  "functional_safety": "ASIL or SIL integrity level if stated (e.g. ASIL B, ASIL D, ASIL B(D), SIL 2, QM); empty string if not stated",
...
- purchasing — procurement, supplier selection, or supply-chain requirements
- process    — requirements on engineering or management processes
- quality    — quality assurance or quality management requirements
- (use additional domain labels when the section heading or requirement text clearly indicates a
  domain not listed above; keep labels lowercase and singular)
...
const testSystemPrompt = `You are a test-case extraction tool for systems engineering test standards and test specifications.

Extract discrete test cases, test procedures, and test steps described in the text supplied by the user.
A test case is any described procedure, check, measurement, or verification activity — whether stated
normatively (shall be tested, must be verified) or procedurally (apply X, measure Y, check Z).

Ignore purely descriptive text, rationale, scope statements, and definitions that do not describe a
specific test to be performed.

Output a JSON array — one object per test case. If there are none, output [].
Output ONLY the JSON array — no prose, no code fences.

## Output schema (one JSON object per test case)

{
  "section":        "nearest section heading in the supplied text, if discernible; otherwise empty string",
  "title":          "one-sentence summary of the test in active voice, including any condition (≤20 words)",
  "item":           "the component, subsystem, or system under test; empty string if not discernible",
  "test_type":      "one of: functional, performance, stress, interface, safety, regression, boundary, environmental, or other",
  "method":         "primary test execution method: T (test/execution), A (analysis), I (inspection), D (demonstration)",
  "preconditions":  "brief summary of required initial state or setup; empty string if not stated",
  "pass_criterion": "brief summary of the condition that defines a pass; empty string if not stated",
  "domain":         "one or more of: system, hardware, software (comma-separated if multiple)"
}

## Method taxonomy

- T — the test is executed on real hardware or software (stimulus applied, response measured)
- A — the test is evaluated by analytical calculation or simulation
- I — the test is evaluated by visual or documentary inspection
- D — the test is verified by operating the item and observing behaviour without instrumentation

## Domain taxonomy

- system   — tests the integrated system or cross-cutting behaviour
- hardware — tests hardware components or HW-specific behaviour
- software — tests software modules, firmware, or SW-specific behaviour

## Rules

- If a single prose block describes multiple distinct test cases (compound test), split it into one
  JSON object per atomic test case. Each split entry is independent and complete on its own.
- Summarize the full test — including any condition ("when X", "if Y"), stimulus, and expected
  outcome — into the "title" field in plain active-voice English.
- For the "section" field, use the nearest heading present in the supplied text.
- Leave "preconditions" and "pass_criterion" as empty strings if the text does not state them.`
...
func assembleTOON(reqs []requirement) string {
	var sb strings.Builder
	fmt.Fprintf(&sb, "requirements[%d]{Requirement|Section|Item|Domain|Verb|Verification|FunctionalSafety}:\n", len(reqs))
...
// assembleTestTOON builds the TOON test-cases table from all extracted test cases
// in document order. IDs must already be set on each test case.
func assembleTestTOON(tests []testCase) string {
	var sb strings.Builder
	fmt.Fprintf(&sb, "tests[%d]{Test|Section|Item|Type|Method|Domain}:\n", len(tests))

	for _, tc := range tests {
		row := fmt.Sprintf("  %s | %s | %s | %s | %s | %s",
			pipeEscape(tc.Title),
			pipeEscape(tc.Section),
			pipeEscape(tc.Item),
			pipeEscape(tc.TestType),
			pipeEscape(tc.Method),
			pipeEscape(tc.Domain),
		)
		sb.WriteString(row)
		sb.WriteByte('\n')
	}
...
// assembleTestJSON serialises the test cases as a pretty-printed JSON array.
// IDs must already be set on each test case.
func assembleTestJSON(tests []testCase) string {
	if tests == nil {
		tests = []testCase{}
	}
